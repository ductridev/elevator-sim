...
export class Dispatcher {
  ...
  handleHallCall(floor: number, direction: UpDown): number {
    ...
    for (const e of this.building.elevators) {
      ...
      const idle = e.direction === IDLE || !e.hasTargets();
      // Lower cost wins; on a tie, prefer an idle cabin over a committed one so
      // a second call doesn't pile onto a cabin that's already busy.
      if (cost < bestCost || (cost === bestCost && idle && !bestIdle)) {
        bestCost = cost;
        best = e;
        bestIdle = idle;
      }
  ...
  #cost(elevator: Elevator, floor: number, dir: UpDown): number {
    ...
    const high = Math.max(cur, ...targets);
    const low = Math.min(cur, ...targets);

    if (ed === UP) {
      // Same way (up) and the call is still above: go straight there.
      if (dir === UP && floor >= cur) return floor - cur;
      // Down call: climb to the top of the sweep (extend it if the call is
      // higher than any current target), then come back down to it.
      if (dir === DOWN) {
        const top = Math.max(high, floor);
        return top - cur + (top - floor);
      }
      // Up call we've already passed: top, down to the bottom (extend if the
      // call is lower), then back up to it.
      const bottom = Math.min(low, floor);
      return high - cur + (high - bottom) + (floor - bottom);
    }
    ...
    const top = Math.max(high, floor);
    return cur - low + (top - low) + (top - floor);
  }
}
